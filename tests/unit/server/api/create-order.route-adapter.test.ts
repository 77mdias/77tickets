import { describe, expect, test, vi } from "vitest";
import { createUnauthenticatedError } from "../../../../src/server/application/errors";
import { createCreateOrderRouteAdapter } from "../../../../src/server/api/orders/create-order.route-adapter";

const SESSION_CUSTOMER = { userId: "57d1cfdb-a4dd-4af8-90be-6ce315f8f6f5", role: "customer" as const };

describe("createCreateOrderRouteAdapter", () => {
  test("extracts session, injects customerId server-side, and forwards to handler", async () => {
    const getSession = vi.fn(async () => SESSION_CUSTOMER);
    const handleCreateOrder = vi.fn(async () => ({
      status: 200 as const,
      body: { data: { orderId: "ord_001" } },
    }));

    const adapter = createCreateOrderRouteAdapter({ getSession, handleCreateOrder });

    const response = await adapter(
      new Request("http://localhost/api/orders", {
        method: "POST",
        body: JSON.stringify({
          eventId: "2f180791-a8f5-4cf8-b703-0f220a44f7c8",
          customerId: "00000000-0000-0000-0000-000000000999",
          items: [{ lotId: "4b021be4-4cb2-4f5f-bcf4-f8237bcb4e7e", quantity: 1 }],
        }),
        headers: { "content-type": "application/json" },
      }),
    );

    expect(response.status).toBe(200);
    await expect(response.json()).resolves.toEqual({ data: { orderId: "ord_001" } });
    expect(handleCreateOrder).toHaveBeenCalledWith({
      actor: { role: "customer", userId: SESSION_CUSTOMER.userId },
      body: {
        eventId: "2f180791-a8f5-4cf8-b703-0f220a44f7c8",
        customerId: SESSION_CUSTOMER.userId,
        items: [{ lotId: "4b021be4-4cb2-4f5f-bcf4-f8237bcb4e7e", quantity: 1 }],
      },
    });
  });

  test("returns 400 when request body is invalid JSON", async () => {
    const getSession = vi.fn(async () => SESSION_CUSTOMER);
    const adapter = createCreateOrderRouteAdapter({
      getSession,
      handleCreateOrder: async () => {
        throw new Error("handler should not be called");
      },
    });

    const response = await adapter(
      new Request("http://localhost/api/orders", {
        method: "POST",
        body: "{invalid-json}",
        headers: { "content-type": "application/json" },
      }),
    );

    expect(response.status).toBe(400);
    await expect(response.json()).resolves.toEqual({
      error: { code: "validation", message: "Invalid request payload" },
    });
  });

  test("returns 401 when session is invalid or expired", async () => {
    const getSession = vi.fn(async () => {
      throw createUnauthenticatedError("Sessão inválida ou expirada");
    });
    const adapter = createCreateOrderRouteAdapter({
      getSession,
      handleCreateOrder: async () => {
        throw new Error("handler should not be called");
      },
    });

    const response = await adapter(
      new Request("http://localhost/api/orders", {
        method: "POST",
        body: JSON.stringify({ eventId: "evt_1", items: [] }),
        headers: { "content-type": "application/json" },
      }),
    );

    expect(response.status).toBe(401);
    await expect(response.json()).resolves.toMatchObject({
      error: { code: "unauthenticated" },
    });
  });

  test("returns 429 when rate limit is exceeded", async () => {
    const getSession = vi.fn(async () => SESSION_CUSTOMER);
    const handleCreateOrder = vi.fn(async () => ({
      status: 200 as const,
      body: { data: { orderId: "ord_001" } },
    }));

    const adapter = createCreateOrderRouteAdapter({
      getSession,
      handleCreateOrder,
      checkRateLimit: () => ({ allowed: false, retryAfterSeconds: 42 }),
      rateLimitMaxRequests: 10,
    });

    const response = await adapter(
      new Request("http://localhost/api/orders", {
        method: "POST",
        body: JSON.stringify({
          eventId: "2f180791-a8f5-4cf8-b703-0f220a44f7c8",
          items: [{ lotId: "4b021be4-4cb2-4f5f-bcf4-f8237bcb4e7e", quantity: 1 }],
        }),
        headers: { "content-type": "application/json", "x-forwarded-for": "203.0.113.4" },
      }),
    );

    expect(response.status).toBe(429);
    expect(handleCreateOrder).not.toHaveBeenCalled();
    expect(response.headers.get("retry-after")).toBe("42");
    expect(response.headers.get("x-ratelimit-limit")).toBe("10");
    expect(response.headers.get("x-ratelimit-remaining")).toBe("0");
    await expect(response.json()).resolves.toMatchObject({
      error: {
        code: "rate_limited",
      },
    });
  });
});
